...
{	
	...
	$order['order_amount'] = $amount - floatval($_POST['money_paid']) - floatval($_POST['discount']) - $order['integral'] - $order['bonus'];
	...
	tendercall($order['order_id'],$order['user_id'],$order['pay_id'],$order['pay_note'],$order['order_amount'],$order['bonus'],$order['bonus_id'],$order['surplus'],$order['integral']);	
...
else if ($_GET['act']=='detail')//订单修改
{
	...
	foreach($order_goods as $k => $val)
	{
		...
		if(($val['goods_id'] != 60 && $val['goods_id'] != 61) || $val['goods_discount'] == -1)
		{
			...
			$cake['goods_attr'] = $val['goods_attr'];
...
elseif ($_GET['act']=='update')
{
	...
	{
		$address = $_POST['address'];
	    $sql = "SELECT route_id FROM ship_area WHERE locate(area_name,'{$address}') > 0";
		$res = $db->getOne($sql);
		if($_POST['country'] == '443')
		{
		    if($_POST['city'] == '742')
	...
	if($_POST['fore_bonus'] != $bonus_str)
	{
		...
		{
		   $db->query("update ecs_user_bonus set used_time = 0, order_id = 0 where bonus_id = '".$val."'");
		}
